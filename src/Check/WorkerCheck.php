 ...
 * precisely the lockout this check exists to catch. Such a row keeps a
...
final readonly class WorkerCheck implements DiagnosticInterface
{
     ...
     * recycling on its time limit, short enough that an operator watching an
     * install sees the verdict.
     */
     ...
     * again. This is the component default; an app that overrides it in its
     * transport DSN has to change this alongside it.
     *
    ...
    public function __invoke(): Diagnostic
    {
        ...
        try {
            ...
            $this->logger->warning('The message queue could not be read.', ['exception' => $e]);

            return new Diagnostic(
                'worker',
                DiagnosticState::Unknown,
                'check.worker.queue_unreadable',
        ...
        if (0 === $pending) {
            if ($claimed > 0) {
                return new Diagnostic(
                    'worker',
                    DiagnosticState::Unknown,
                    'check.worker.claimed_in_flight',
                    ['%count%' => $claimed],
